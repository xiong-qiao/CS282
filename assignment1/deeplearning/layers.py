...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (x_hat, gamma, xmean, ivar, sqrtvar, var, eps)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  elif mode == 'test':
...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  x_hat, gamma, xmean, ivar, sqrtvar, var, eps = cache
  N, D = dout.shape

  # when compute forward pass, we use element-wise multiplication, in the backward, 
  # we should also use element-wise multiplication
  ...
  # xhat = xmean * ivar
  divar = np.sum(dxhat * xmean, axis=0)
  dxmean1 = dxhat * ivar
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  # There are total N input data to deal with at one time
  for n in range(N):
    ...
    for f in range(F):
      for height in range(Hout):
        for width in range(Wout):
          starh = height * stride
          starw = width * stride
          out[n, f, height, width] = np.sum(x_padded[n, :, starh : HH + starh, 
                                        starw : WW + starw] * w[f, :, :, :]) + b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
  ...
  for n in range(N):
    for height in range(Hout):
      for width in range(Wout):
        # There are F total different filters
        for f in range(F):
  ...
  # dw shoule be of size (F, C, HH, WW)
  dw = np.zeros(w.shape)
  x_padded = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant', constant_values=0)
  for n in range(N):
    for f in range(F):
      for height in range(Hout):
        for width in range(Wout):
          starth = height * stride
          startw = width * stride
          dw[f, :, :, :] += dout[n, f, height, width] * x_padded[n, :, starth : HH + starth,
              startw : WW + startw]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_forward_naive(x, pool_param):
  ...
  switches = np.empty(out.shape, dtype=np.int32)
      ...
      for h in range(Hout):
        for w in range(Wout):
          ...
          out[n, c, h, w] = np.max(x[n, c, 
            starth : starth + pool_height, startw : startw + pool_width])

          # Use switches to keep track of the index which will be used during backpropagation
          switches[n, c, h, w] = np.argmax(x[n, c, 
            starth : starth + pool_height, startw : startw + pool_width])
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_backward_naive(dout, cache):
  ...
  # dx should be the same shape as x
  dx = np.zeros(x.shape)
    ...
    for c in range(C):
      for h in range(Hout):
        for w in range(Wout):
          starth = h * stride
          startw = w * stride
          oneD_index = switches[n, c, h, w]
          h_index = int(oneD_index / pool_width)
          w_index = oneD_index % pool_width
          dx[n, c, starth + h_index, startw + w_index] = dout[n, c, h, w]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
