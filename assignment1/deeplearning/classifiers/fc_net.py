...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        reg = self.reg
        N, D = X.shape[0], W1.shape[0]
        
        hidden_layer = np.maximum(np.dot(X.reshape(N, D), W1) + b1, 0)
        ...
        # Loss with respect to relu
        dhidden[hidden_layer <= 0] = 0
        
        grads['W1'] = np.dot(X.reshape(N, -1).T, dhidden)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for index in range(self.num_layers):
      nrows = all_dims[index]
      ncols = all_dims[index + 1]

      layer_name = "%d" % (index + 1)
      W_name = "W" + layer_name
      b_name = "b" + layer_name
      self.params[W_name] = weight_scale * np.random.randn(nrows, ncols)
      self.params[b_name] = np.zeros(ncols)

      if use_batchnorm and index < (self.num_layers - 1):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers + 1):
      ...
      if i == self.num_layers:
        # handle the final layer where there is affine only
